<?php
/**
 * Adds function to upload local avatars
 *
 * @since      1.1
 *
 * @package    Iis_Pack
...
class LocalAvatars {
	...
	public function edit_user_profile( $profileuser ) {
		?>
		<h3><?php _e( 'Local profile image. Replaces your Gravatar image for this site only.', 'iis-pack' ); ?></h3>
				...
				<td>
					...
					if ( empty( $options['iis_pack_simple_local_avatars_caps'] ) || current_user_can( 'upload_files' ) ) {
						...
						if ( empty( $profileuser->simple_local_avatar ) ) {
							...
							echo '<span class="description">' . __( 'You do not have media management permissions. To change your local profile picture, contact the blog administrator.','iis-pack' ) . '</span>';
						}
					}
				?>
				</td>
